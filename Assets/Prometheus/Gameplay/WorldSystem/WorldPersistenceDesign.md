...
需求是把 POI 的**可变状态**（解锁/开启/收集/击败等）托管到**服务器**，客户端不再直接改状态，而是**发请求、服务器确认后才做表现**。当前无真实网络，服务器用**同进程模拟**（JSON 模拟数据库）。
...
当前 `PoiSyncService` 同进程直接调用。接入真实网络时：
- 用网络层替换 `PoiSyncService.RequestApply` / `PullAll` 的实现（RPC 发到服务器）。
- 服务器侧保留同一套 `PoiState` / 校验逻辑，换真实数据库实现 `IPoiDataStore`。
- 客户端 `TryInteract` 由同步返回改为异步回调（true 时仍走 `PoiInteractionHandler.Apply`）。
