...
- `IAnimationHost` 是播放会话对宿主的最小要求（会话是否仍生效、宿主世界坐标、会话结束通知）。本目录因此不认识 `SpineComponent`、`Entity` 等任何实体模型类型，可以脱离 Entity 单独测试。
- `SpineComponent`（位于 `EntitySystem/Character/Component/`）实现 `IAnimationHost`，是唯一播放仲裁组件，负责优先级、主轨所有权、序列播放和一次性会话。
...
- `AttackComponent.attackHits` 与 `AnimationLibrary.atkExecutor.attacks` 按下标一一对应，组件的每段 Inspector 配置只保留 `ColliderProxy` 和 `abilityId`；数值由 `TalentConfig.normalAttack.stages` 的同一下标提供，命中伤害按 `Max(0, Atk × damageMultiplier × TalentScale + damageOffset)` 生成（只到「倍率 × 缩放属性」为止，暴击、增伤、防御区与元素反应统一由 `Combat/DamagePipeline` 在结算时处理），其他三种能力使用相同的天赋倍率顺序。
