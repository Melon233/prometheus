...
namespace Xuan.Prometheus.World.Tests
{
    ...
    public sealed class InteractSmokeTests
    {
        ...
        {
            Assert.That(PoiSystem.GetInteractOp(PoiType.TeleAnchor), Is.EqualTo(PoiOp.Unlock));
            Assert.That(PoiSystem.GetInteractOp(PoiType.Statue), Is.EqualTo(PoiOp.Unlock));
            Assert.That(PoiSystem.GetInteractOp(PoiType.Dungeon), Is.EqualTo(PoiOp.Unlock));
            Assert.That(PoiSystem.GetInteractOp(PoiType.Chest), Is.EqualTo(PoiOp.OpenChest));
            Assert.That(PoiSystem.GetInteractOp(PoiType.SpiritCore), Is.EqualTo(PoiOp.CollectCore));
            Assert.That(PoiSystem.GetInteractOp(PoiType.Gathering), Is.EqualTo(PoiOp.Gather));
            Assert.That(PoiSystem.GetInteractOp(PoiType.MapBoss), Is.EqualTo(PoiOp.Defeat));
        }
    }
}
