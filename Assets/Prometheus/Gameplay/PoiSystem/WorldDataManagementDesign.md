...
> 状态：**早期设计，部分已被取代**。保留原因是其中的建模结论仍然成立，实现路径则已改变。
> 适用范围：单一大场景（无缝开放世界）· 数百到一千个 POI
> 涉及对象：传送锚点、七天神像、宝箱、神瞳、采集物、副本、地图 Boss、怪物营地

## 本文与现行方案的关系

本文写于服务器接入之前，假设 POI 状态由**客户端本地存档**持有。项目随后改为**服务器权威**，因此下列部分只作为设计记录，不再指导实现：

| 本文内容 | 现状 |
| --- | --- |
| §4 `Gameplay/World/` 目录与命名空间规划 | 已由 `Gameplay/PoiSystem/` 取代 |
| §5.1 `WorldPoint` 统一 MonoBehaviour 外壳 | 方向正确，且已被 `ARCH-ENT-005` 确认——POI 不走 ELC，直接用 MonoBehaviour 承载配置与交互入口 |
| §5.4 `WorldPointRegistry` 本地注册表 | 由 `PoiSystem` 的场景扫描 + `Id` 索引取代 |
| §7 `IWorldStateStore` 本地存档 | **已被取代**：状态改为服务器权威，见 [WorldServerArchitecture.md](WorldServerArchitecture.md) 与 [WorldPersistenceDesign.md](WorldPersistenceDesign.md) |
| §10 落地步骤 | 已按服务器权威路线重排，不再适用 |

仍然成立并已落地的结论：

- **Definition / Instance / State 三者分离**，只持久化 State（§2）——现行方案中 Definition 存于导出配置、State 存于 MongoDB，分离关系不变。
- **可刷新对象存绝对世界时间而非相对秒数**（§5.5、§9.3），天然支持离线重生。
- **按空间分区做范围查询**（§5.4）——现行方案用 `chunkId` 分区，思路一致，粒度从 50m 格网改为 20m chunk 且由服务器索引。
- **存档只记 `id → state`，不记场景路径或索引**（§7）——现行方案的语义化 ID 正是这条结论的延续。
...
- 已具备 `Gameplay/EntitySystem`（实体）、`Gameplay/EntitySystem/Character/Config`（配置）、`Gameplay/EntitySystem/Character/Logic`（逻辑）、`UI/Map`（小地图 UI 数学与图标目录）、`Framework/*`（CoreKit/EventKit/AssetKit）等基础模块 —— 新模块应**挂靠这些体系**而非另起炉灶。
