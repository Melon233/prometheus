...
> 对应 Spec：`Assets/Prometheus/Gameplay/PoiSystem/WorldSystemSpec.md`
> 本文档为 `PoiSystem` 的落地设计，描述数据结构、编辑器/烘焙流程、运行时注册与状态同步，以及与现有 `EntitySystem` 的对接方式。
...
- `PoiSystem` 负责 POI 索引与服务器状态同步；消费、解锁与重生状态直接保存在场景的 `PoiMono` 组件上。**POI 不使用 ELC**：数量由场景摆放固定、表现对象先于运行时存在、状态全部来自服务器、且没有任何逐帧逻辑，实体生命周期与组件组合对它是净负担。
...
| 现有组件 | 位置 | 在本系统中的角色 |
|----------|------|-----------------|
| `XSystem` | `Assets/Prometheus/Framework/GameplayKit/XSystem.cs` | `PoiSystem` 的基类，通过 `GameplayKit.AddSystem` 注册 |
...
(场景摆放+配置)             创建 PoiEntity/NpcEntity               │
      │                     注册到 EntitySystem                    ▼
      └----烘焙/导出------>  服务器静态定义               按 PoiId 应用到PoiMono
                                  │                                │
                                  └-----------------------> PoiMono 按类型写入状态并控制显隐
```
...
- **运行时**：`PoiSystem.LoadFromScene` 扫描此组件并读取 `Config`，创建绑定同一场景 GameObject 的 POI Entity；烘焙/导出数据用于服务器静态定义和空间索引。
...
3. **状态显隐**：一次性 POI 在消费后由PoiMono 隐藏；可刷新 POI 的显隐**只在收到服务器状态时判定一次**（`RespawnablePoiLogic.SetRespawnAt`），不存在本地倒计时，也不占用逐帧更新。
4. **网络同步独立**：玩家附近 3x3 chunk 仍用于拉取服务器 POI 状态，但该范围不决定场景对象是否可见。

### 5.2 PoiSystem（核心，已实现于 `Gameplay/PoiSystem/PoiSystem.cs`）
...
internal sealed class PoiSystem : XSystem, IPoiSystem
{
    ...
    private static IServiceSystem ServiceSystem => Core.Gameplay.GetSystem<IServiceSystem>(); // 会话状态：是否已进入世界、是否断线。
    private static IPoiGateway Gateway => Core.Gameplay.GetSystem<IPoiGateway>();          // 世界领域的网络适配器。
...
- 玩家位置在 `OnUpdate` 中取当前上场成员（`Core.Gameplay.GetSystem<ITeamSystem>().ActiveMember`，本文件内封装为私有属性 `ActivePlayer`）的 `bindGo` 坐标。
- Entity 注册、敌人生成和营地补刷统一通过 `Core.Gameplay.GetSystem<IEntitySystem>()` 完成。

> 当前实现的服务器接入流程以场景 `PoiMono` 为静态定义来源：组合根先注册 ServiceSystem 再注册 PoiGateway，PoiSystem 在调用点分别解析二者（会话状态取自 `IServiceSystem`，网络请求走 `IPoiGateway`）；`AfterNew` 注册怪物营地死亡监听并生成初始史莱姆，`InitializeAsync` 首先扫描场景并建立本地 POI 索引，保证地图和交互列表可以独立于网络显示；随后调用业务接口 `EnterWorldAsync`，由 ServiceSystem 内部完成连接与 JoinRoom，再恢复玩家坐标并启用服务器同步。PoiSystem 的全部异步调用都传入自身生命周期令牌，释放后 continuation 不再修改集合或 Unity 对象。ServiceSystem 把 NetworkKit 意外断线转换为世界不可用通知，PoiSystem 随即停止上传、拉取和交互；本地 POI 仍保留。

### 5.3 注册与显隐
- **初始化**：`LoadFromScene` 为每个合法 `PoiMono` 创建 `PoiEntity` 或 `NpcEntity`，注册到 EntitySystem 并保留场景 GameObject。
- **显隐**：PoiSystem 不再依据玩家距离统一控制显隐。一次性消费、可刷新冷却和重生由具体 POI Logic 调用 `SetPoiVisible`。
- **生命周期**：PoiSystem 释放前，POI Entity 仍由 EntitySystem 统一托管；GameplayKit 销毁时先释放全部 Entity，再逆序释放其他 System。

### 5.4 玩家位置来源
`PoiSystem` 需要玩家世界坐标用于位置上传与附近 chunk 状态同步，经确认**复用 `TeamSystem` 现成成员**，不再另设位置定位器：
...
{
    // 由 PoiSystem 已实例化预制体后构造（与 PlayerEntity/SlimeEntity 的 bindGameObject 模式一致）
...
- 可刷新三类（采集物/地图Boss/怪物营地）共用 **`PoiMono.SetRespawnAt`** 基类：服务器保存绝对重生时间戳（Unix 毫秒），客户端在 `SetRespawnAt` 中判定一次可用性并同步显隐。**客户端没有倒计时，也不做逐帧判定**；离线期间流逝的时间由服务器时间戳天然覆盖。
- 各类型由 `PoiMono` 创建（`PoiLogicFactory.cs`）；全局状态事实统一通过 `Core.Event` 发布，载荷实现 `IEvent` 且构造后不可变（`PoiEvents.cs`：`PoiOpenedEvent`/`PoiCollectedEvent`/`PoiGatheredEvent`/`PoiUnlockedEvent`/`PoiDefeatedEvent`）。

| 类型 | 公开行为 | 状态 | 广播事件 |
|------|---------|------|---------|
| `PoiMono` | 暂无交互 | 营地实体由 `PoiSystem` 维护 | 史莱姆死亡由 `Core.Event` 广播 |

> 业务数据（奖励表、战斗、传送目标、副本场景）依赖尚未存在的奖励/战斗/传送系统，此处仅实现**生命周期状态、幂等切换、重生计时与事件广播**；真实业务在 `ChestLogic.Open()` 等公开方法处接入。

宝箱开启和采集物采集由服务器确认成功后，`PoiSystem` 先通过叙事系统播放同一套运行时镜头序列：演出镜头从当前构图平滑靠近并对准目标 POI，再恢复原构图；演出在整个时段屏蔽玩法输入、快捷键、HUD 点击和 UI 导航。该链路随 FilmSystem 移除后尚未在 NarrativeSystem 上重新接入。演出自然结束后才应用服务器返回的宝箱已开启状态或采集物冷却状态，避免特写期间目标提前消失。
...
|---------|------------|-----------|-------------|
| TeleAnchor | `TeleAnchorConfig` | Statue | `StatueConfig` | Chest | `ChestConfig` | SpiritCore | `SpiritCoreConfig` | Gathering | `GatheringConfig` | Dungeon | `DungeonConfig` | MapBoss | `MapBossConfig` | MonsterCamp | `MonsterCampConfig` | `PoiMono` | `PoiSystem` 按每个场景营地实例生成一只史莱姆；史莱姆首次致死后通过 `Core.Event` 通知并在同帧安全阶段于原营地位置补刷一只 |

> 采集物与地图 Boss 通过共享基类 `PoiMono.SetRespawnAt` 处理重生：状态完全由服务器时间戳驱动，判定时机是 chunk 状态拉取与交互响应两条链路（均经 `PoiStateApplier`）。怪物营地的史莱姆由 `PoiSystem` 监听全局死亡事件后立即补刷。PoiSystem 不再按距离管理 POI 显隐。
>
> **已知缺口**：`PoiSystem.syncedChunks` 目前从不清除，每个 chunk 每局只拉取一次。因此 POI 进入冷却后，本局不会再收到该 chunk 的新状态，也就不会重新显示。需要补充 chunk 重新同步策略（离开后重入时移出 `syncedChunks`，或按间隔重拉）才能让重生真正生效。
...
```
玩家移动 -> PoiSystem.OnUpdate（低频，默认 0.25s 一跳）
  1. 取 ActivePlayer（当前上场成员）位置
  2. 按固定间隔向服务器上传玩家位置
...
| **P2 PoiSystem** | 场景 POI 注册、位置上传、附近 3x3 chunk 状态同步 | ✅ 已实现 |
...
| 可刷新类重生 | 通过共享基类 `PoiMono.SetRespawnAt` 实现（`Consume` → 按 `Config` 周期倒计时 → 自动重生），**不抽独立 `RespawnScheduler`** | §6.3 各类型 Logic |

## 12. 后续 / 待实现时对齐

- `PoiSystem` 已接入玩法组合根 `PrometheusSystemInstaller.RegisterSystems`（`Bootstrap/PrometheusSystemInstaller.cs`），随玩法开局自动注册。烘焙资产需纳入 YooAsset 包内地址 `Assets/Prometheus/Gameplay/PoiSystem/Data/WorldRegionsConfig.asset`；缺失时降级为空世界并告警。
...
`PoiSystem` 同时是当前世界地图的唯一运行时数据出口。`AfterNew` 通过 AssetKit 读取 `WorldMapDefinition`，对外提供 `MapTexture`、`MapWorldLength`、`MapWorldWidth`、`MapInitialZoom`、`MapZoom`、`WorldToMapNormalized`、`TryGetPlayerPosition` 和只读 `AllPois`；HUD 小地图和 `MapPanel` 不直接访问网络客户端或 POI 内部逻辑。
...
大地图的传送请求也由 `PoiSystem.TryTeleportToPoi` 统一处理。该接口只接受当前已加载且类型为 `Statue` 或 `TeleAnchor` 的 POI；传送时暂时停用玩家 `CharacterController`、写入目标位置并清空移动速度与 Root Motion，避免下一帧运动逻辑覆盖传送结果。随后 HUD 和大地图直接读取同一份实体 Transform，保证两个视图继续使用同一份世界坐标事实。
### NPC POI（第一阶段）

当 `PoiConfig.PoiType` 为 `Npc` 时，PoiSystem 使用 `PoiConfig.Npc` 创建 `NpcEntity` 并常驻注册到 EntitySystem，不再依据玩家距离激活或回收。NPC 身份与行为由 NpcSystem 的 `NpcDefinition`、`NpcComponent` 和 `NpcLogic` 提供；PoiSystem 不直接控制对话、镜头或任务推进。
