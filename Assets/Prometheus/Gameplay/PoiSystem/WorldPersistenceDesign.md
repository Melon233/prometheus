...
> 关联：`WorldSystemSpec.md` / `PoiSystem.md`
...
- 再通过 `Core.Gameplay.GetSystem<IServiceSystem>().EnterWorldAsync` 进入默认世界；ServiceSystem 在该业务流程内部完成一次服务器连接检测与 JoinRoom 请求，成功后 PoiSystem 启用位置上传、区块状态同步和权威交互。
...
```
PoiSystem.TryInteract(entity, op)
...
└─ PoiSystem.cs                   // 按 PoiId 同步 + TryInteract 请求确认
```
...
- **Rotation 仅存储暂不应用**：`PoiConfig.Rotation` 已导出记录，运行时绑定 `PoiEntity` 时暂未设置 `bindGo` 旋转；后续需要时在 `PoiSystem.LoadFromScene` 中应用即可。
...
客户端 `ServiceSystem` 通过 `Framework/NetworkKit` 的 Transport → Framing → Protocol → Session/RPC 分层访问服务器，并让全部 Gameplay 业务请求与 Push 复用同一个 `INetworkClient` 会话。NetworkKit 只管理连接和通用 Packet，不解释任何业务 Body；ServiceSystem 负责进入世界与通用请求通道；chunk 拉取、POI 交互和坐标上传等世界业务协议由 `PoiGateway` 组包解包。World 经 `IPoiGateway` 调用业务接口、经 `IServiceSystem` 读取会话状态；它先扫描本地场景，再进入世界，失败时保留本地地图和场景 POI，只关闭网络同步与服务器交互。

## 接口边界

`PoiSystem` 是程序集内部实现，由 `GameplayKit` 以 `IPoiSystem` 注册。网络通信由 `IPoiGateway` 提供，PoiSystem 不持有网络客户端，也不再向 Bag 转发库存请求。`BagSystem` 使用 `IBagGateway.GetItemsAsync` 并独立维护背包快照；两个 Gateway 共享 ServiceSystem 的同一条连接，ServiceSystem 在所有 Gateway 与消费者之后释放底层连接。

PoiSystem 与 BagSystem 不通过构造函数注入 ServiceSystem，而是在使用点经 `Core.Gameplay` 查询接口。两者把自身生命周期令牌传入 ServiceSystem，释放时取消未完成请求；ServiceSystem 收到 NetworkKit 意外断线后切换为世界不可用，PoiSystem 停止后续网络轮询。当前不自动重连。
