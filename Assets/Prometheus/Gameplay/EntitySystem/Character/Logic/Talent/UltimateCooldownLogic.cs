...
{
    /// <summary>独立推进玩家大招冷却，使 Stun、Silence、跳跃或动作阻塞都不会暂停冷却。</summary>
    public sealed class UltimateCooldownLogic : Logic
    {
        private UltimateComponent ultimateComponent;

        /// <summary>缓存冷却组件并初始化每个玩家实体独立拥有的冷却运行态。</summary>
        public override void AfterNew()
        {
            OrderTag = OrderTag.Gameplay;
            ControlRequirement = LogicControlRequirement.None;
            if (!Entity.TryGetComp(out ultimateComponent)) throw new InvalidOperationException("UltimateCooldownLogic requires UltimateComponent.");
        ...
        {
            ultimateComponent.AdvanceCooldown(dt);
        }
