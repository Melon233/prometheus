...
{
    /// <summary>
    /// 世界大地图面板：使用 PoiSystem 提供的静态地图和 POI 数据，在独立视口中支持拖拽、滚轮缩放和标记显示。
    ...
    public sealed class MapPanel : MapPanelBase
    {
        ...
        /// <summary>保存最近一次 PoiSystem 发布的玩家位置。</summary>
        private Vector3 playerPosition;

        /// <summary>标记是否已经接收过玩家位置。</summary>
        private bool hasPlayerPosition;

        /// <summary>当前地图内容缩放倍数；实际值从 PoiSystem 的单局缓存读取。</summary>
        ...
        /// <summary>打开面板时解析 PoiSystem、订阅地图事实并立即重放当前地图状态。</summary>
        protected override void OnOpen()
        {
            if (!Core.Gameplay.TryGetSystem(out worldSystem)) throw new InvalidOperationException($"{nameof(MapPanel)} requires {nameof(IPoiSystem)}.");
            if (!Core.Gameplay.TryGetSystem(out worldMapSystem)) throw new InvalidOperationException($"{nameof(MapPanel)} requires {nameof(IWorldMapSystem)}.");
            Core.Event.AddListener<WorldMapReadyEvent>(OnWorldMapReady);
            Core.Event.AddListener<WorldMapPoiChangedEvent>(OnWorldMapPoiChanged);
            float restoredZoom = MapPanelViewState.Zoom > MapPanelViewState.UnsetZoom ? MapPanelViewState.Zoom : worldMapSystem.InitialZoom;
            zoom = Mathf.Clamp(restoredZoom, MinimumZoom, MaximumZoom);
        ...
        /// <summary>关闭面板时解除地图事件监听，避免面板销毁后继续接收 PoiSystem 通知。</summary>
        ...
        {
            if (mapImage == null || worldMapSystem == null) return;
            mapImage.texture = worldMapSystem.Texture;
            float aspect = worldMapSystem.WorldLength > 0f && worldMapSystem.WorldWidth > 0f ? worldMapSystem.WorldLength / worldMapSystem.WorldWidth : 1f;
        ...
        {
            if (!hasPlayerPosition || viewport == null || mapContent == null || worldMapSystem.Texture == null) return;
            Vector2 playerUv = worldMapSystem.WorldToNormalized(playerPosition);
        ...
        /// <summary>重新创建地图上的玩家和 POI 标记，标记位置统一通过 PoiSystem 的地图坐标接口换算。</summary>
        private void RebuildMarkers()
        {
            ClearMarkers();
            if (worldMapSystem.Texture == null) return;
            if (hasPlayerPosition) CreateMarker("Player", WorldMapIconCatalog.LoadPlayerIcon(), worldMapSystem.WorldToNormalized(playerPosition), new Vector2(40f, 40f), false, null);
        ...
        {
            Core.Event.RemoveListener<WorldMapReadyEvent>(OnWorldMapReady);
            Core.Event.RemoveListener<WorldMapPoiChangedEvent>(OnWorldMapPoiChanged);
        }
        ...
        /// <summary>POI 状态变化时重新读取 PoiSystem 的 POI 集合。</summary>
        ...
        /// <summary>点击神像或传送锚点图标后请求 PoiSystem 执行传送，成功后关闭大地图。</summary>
