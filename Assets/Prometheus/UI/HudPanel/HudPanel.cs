...
namespace Xuan.Prometheus
{
    ...
    public sealed class HudPanel : HudPanelBase
    {
        ...
        /// <summary>订阅 PoiSystem 的地图资源和 POI 状态事实；玩家坐标由逐帧实体读取驱动。</summary>
        ...
        /// <summary>解除 HUD 小地图的 PoiSystem 监听，避免缓存面板关闭后继续更新已隐藏控件。</summary>
        ...
        /// <summary>根据 PoiSystem 地图定义设置纹理、局部视口和 POI 标记；地图未拍摄时显示空白视图。</summary>
        private void RefreshMinimap()
        {
            if (minimapImage == null || worldMapSystem == null) return;
            minimapImage.texture = worldMapSystem.Texture;
            minimapImage.enabled = worldMapSystem.Texture != null;
            if (worldMapSystem.Texture == null)
            {
            ...
            {
                Vector2 playerUv = worldMapSystem.WorldToNormalized(minimapPlayerPosition);
        ...
        /// <summary>按 PoiSystem 当前 POI 集合重建可见于小地图视口内的彩色标记。</summary>
        ...
        private void OnInteractClick(PoiConfig config)
        {
            ...
            if (worldSystem.TryGetPoi(config.Id, out PoiMono poi)) poi.OnInteract();
        }
