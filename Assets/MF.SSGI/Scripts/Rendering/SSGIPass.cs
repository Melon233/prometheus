...
namespace MF.SSGI {
    public class SSGIPass : ScriptableRenderPass {
        ...
        // URP 17 compatibility mode exposes camera attachments as RTHandles whose lifetime is limited to ScriptableRenderPass execution, so callers must resolve them inside the Execute call chain instead of caching them across cameras or frames.
        ...
        private void CaptureScreenAndLight(ScriptableRenderContext context, RenderingData renderingData, float limitedResScale) {
            ...
            cmd.Blit(cameraColorTarget, nameID);
            rtsToRelease.Add(nameID);
            ...
            cmd.Blit(cameraColorTarget, nameID, captureLightMaterial);
            rtsToRelease.Add(nameID);
        ...
        private void BlitToScreen(ScriptableRenderContext context, RenderingData renderingData, float limitedResScale, SSGIVolumeComponent ssgiComp) {
            ...
            cmd.Blit(null, cameraColorTarget, blitFinalImageMaterial);
