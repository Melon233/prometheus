...
默认扫描 `Assets/BundleResources/Config` 和 `Assets/`，排除 `Plugins`、`Trd`、`ThirdParty`、`Tests` 和 `Test` 路径。索引保存于 `Library/Prometheus/ConfigCenter/config-index.json`，属于可重建的编辑器派生数据。
...
为保证大量配置下的交互性能，扫描根目录在编辑器会话内缓存，目录树只在索引或根目录变化时重建，目录排序、后代数量和搜索命中结果也在对应状态变化时统一计算。折叠状态启动时从 `EditorPrefs` 载入内存，之后只在用户实际切换时持久化；滚动区域使用固定行高的扁平列表，并只绘制当前视口附近的目录和配置行，因此鼠标悬浮与滚动不会随全部展开条目数量执行等量 GUI 绘制。
